...
1. Introducing the new unified wildfire indicator as additional indicator for forward looking scenario based projections in Climate Explorer and the Sust Global API
    ...
    2. This new unified wildfire indicator is what is surfaced when selecting the hazard `Wildfire` using the drop down on Climate Explorer
    ...
    5. Previously available wildfire inidicators: `burned_area_norm` and `fire_kbdi_susceptability` will continue to be available through the API and the CSV datasets but not available in the dashboard view in Climate Explorer.
2. Introducing the new Unified water stress indicator as additional indicator for forward looking scenario based projections in Climate Explorer and the Sust Global API
    ...
    5. Previously available water stress inidicators: `score` and `spei_norm` will continue to be available through the API and the CSV datasets but not available in the dashboard view in Climate Explorer.
    6. Water stress scores are now capped at an upper limit of 1.0 instead of 4.0. A value of 1.0 indicates that annual water demand is equal to or exceeds the annual renewable supply. Demand can exceed renewable supply when non-renewable water sources like aquifers are accessed. The normalization value for standardizing values between 0.0 and 1.0 is now 1.0 instead of 4.0. This change impacts the magnitude of water stress score values displayed in the Climate Explorer 
...
4. Introducing two new forward looking exposure time series plots and reporting options:
    1. **Decadal summary time series**: As a drop down option above the forward looking time series, we enable a new view, `Decadal Summary`,that presents decadal summaries based on 10 year moving averages at 5 year offsets. This provides a representative view of risk exposure for the selected hazard for the selected scenario.
    2. **Relative Change summary**: As a drop down option above the forward looking time series,we enable a new view, `Relative Change Summary`, that presents relative change based on 30 year averages from the 1980-2010 and the 2020-2050 time windows. We also quantify the relative change as a percentage of past exposure in the dashboard view.
